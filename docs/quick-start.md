...
The prompt is the step's second argument, and its `{{ ticket_message }}` template pulls the run's input from the workflow state. [Agent Steps](agent-steps.md#prompts) covers every prompt form.
...
Definitions are registered at boot on every process, since queue workers need them too. Learn more in [registration](defining-workflows.md#registration).
...
[Defining Workflows](defining-workflows.md) documents the full definition API (conditions, parallel fan-outs, loops, aliases, drift protection) and the [documentation index](README.md) maps everything else, from prompts and state to observability and production operations.
