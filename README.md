...
- **Interruptible** *(in development)* — `awaitHuman()` will park a run for hours or days; `resume()` validates the human's input and continues.

> **Status: pre-release.** The core engine (sequential, conditional, parallel, evaluator steps; checkpoint/retry; events; testing fakes) is implemented and tested. Human interrupts (`awaitHuman`) and agent handoffs are in development. APIs may change before 1.0.
...
| `definition_drift` | `strict` (refuse to resume a changed definition) or `loose` (best-effort by step name). |
